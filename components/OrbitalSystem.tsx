...
const PLANET_COLORS = [
  0xA9A9A9, 0xc1440e, 0xe8cda0, 0x7de8e8,
  0xc88b3a, 0xe4d191, 0x4b70dd, 0xb5b5b5,
  0xa3c96e, 0xd47de8, 0xe87d7d, 0x7de8b0,
];

// Gezegen başına material fine-tuning: [roughness, metalness]
const PLANET_MATERIAL: [number, number][] = [
  [0.6, 0.1], [0.9, 0], [0.7, 0.05], [0.5, 0.3],
  [0.8, 0], [0.6, 0.05], [0.4, 0.2], [0.95, 0],
  [0.75, 0.1], [0.5, 0.15], [0.85, 0], [0.6, 0.1],
];
...
interface Spherical { theta: number; phi: number; r: number; }
...
export default function OrbitalSystem({ projects }: Readonly<OrbitalSystemProps>) {
  ...
  const handlePlanetClick = useCallback(
    (project: Project) => { router.push(`/projects/${project.slug}`); },
    [router]
  );
  ...
  useEffect(() => {
    ...
    // ── Camera ────────────────────────────────────────────────────────────
    const totalSpread = BASE_ORBIT_R + ORBIT_STEP * projects.length;
    ...
    // ── Lighting ──────────────────────────────────────────────────────────
    scene.add(new THREE.AmbientLight(0x1a1a3a, 1));          // soğuk space ambient
    ...
    projects.forEach((project, i) => {
      ...
      const [roughness, metalness] = PLANET_MATERIAL[i % PLANET_MATERIAL.length];

      const orbit = new THREE.Mesh(
        new THREE.RingGeometry(orbitR - 0.015, orbitR + 0.015, 128),
        new THREE.MeshBasicMaterial({ color: 0x334466, side: THREE.DoubleSide, transparent: true, opacity: 0.4 })
      );
      ...
      if (project.featured) {
        const ring = new THREE.Mesh(
          ...
          new THREE.MeshBasicMaterial({ color: 0xffd060, side: THREE.DoubleSide, transparent: true, opacity: 0.3 })
        );
        ring.rotation.x = Math.PI / 2.5;
        mesh.add(ring);
      }

      planetMeshes.push({ mesh, project, angle: Math.random() * Math.PI * 2, orbitR, speed });
    });
    ...
    const onMouseMove = (e: MouseEvent) => {
      ...
      if (hits.length > 0) {
        ...
        if (hit) {
          ...
          setHover({ project: hit.project, screenX: e.clientX, screenY: e.clientY });
          return;
        }
      }
    ...
    const spherical: Spherical = { theta: 0, phi: Math.PI / 4, r: totalSpread * 1.4 };
    ...
    const onDrag      = (e: MouseEvent) => {
      ...
      spherical.phi    = Math.max(0.1, Math.min(Math.PI / 2.1, spherical.phi - (e.clientY - prevMouse.y) * 0.005));
      prevMouse = { x: e.clientX, y: e.clientY };
    };
    const onWheel = (e: WheelEvent) => {
      spherical.r = Math.max(10, Math.min(totalSpread/projects.length*9, spherical.r + e.deltaY * 0.08));
      e.preventDefault();
    };
    ...
    <div className={styles.wrapper}>
     <header className={styles.header}>
        <div className={styles.headerLeft}>
          ...
          <span className={styles.subtitle}>{projects.length} projects · click the plant</span>
        </div>
        <div className={styles.headerRight}>
          {hover && <span className={styles.hoveredLabel}>{hover.project.title}</span>}
          <button className={styles.closeBtn} onClick={() => router.push("/")} aria-label="Ana sayfaya dön" title="Ana sayfaya dön">✕</button>
        </div>
      </header>
      <div ref={mountRef} className={styles.canvas} />

      {hover && (
        <div className={styles.projectCard} style={{ left: hover.screenX, top: hover.screenY }}>
      ...
      <div className={styles.bottomBar}>
        ...
        <div className={styles.legend}>
          {projects.map((project, i) => (
            <button key={project.id} className={styles.legendItem} onClick={() => router.push(`/${project.slug}`)} title={project.title}>
              <span className={styles.legendDot} style={{ background: `#${PLANET_COLORS[i % PLANET_COLORS.length].toString(16).padStart(6, "0")}` }} />
